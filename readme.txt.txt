November Ajax

Description :
November Ajax is an solution for Categories/Sub-categories, this is a Google Chrome extension and you need to add it in Browser system.

How to add November Ajax to Google Chrome :
1-Open chrome://extensions/
2-Click on load unpacked extension
3-Select November Ajax Folder.
4-Turn on Developper Mode in top right of the page.
Note : I don't wont to share it in Google Chrome App Store because they work for Pampers only.

Functionality :
1-Return list of Urls from NavMenu (Categories, Sub-categories, Reward and Product)
2-Open All Ulrs from NavMenu in other tabs in the same browser

How To Use November Ajax:
1-Open Home Page (e.g : www.pampers.com)
2-Save the source code of the page as a html.
3-Run November Ajax (I configured the code to be run only when you open www.pampers.com)

Result :
1-November Ajax return a list of Urls with the node 'single-link' (Categories, sub-categories, Reward LP, Rewards Catalogue, Product LP)
2-List of Urls is auto copyed and ready to past anywhere.
3-The Urls are auto opened in other tabs in the same browser.

Test can be done with November Ajax : 
1-Verification of $name in NavMenu (return list of urls).
2-Verification of number of element in NavMenu (return count of urls in NavMenu).
3-Verify no UI issue in each categories / sub-categories page(Open all Urls).
4-Verify all categories / sub-categories open correctly.(Open all Urls)

V1.0
20/04/2018
ricky.dovankhac@proximitybbo.fr
Ext : 288